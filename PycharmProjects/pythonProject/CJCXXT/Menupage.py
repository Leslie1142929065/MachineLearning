...
class MenuPage(object):
    def __init__(self, master=None):
        self.root = master
        self.root.geometry('%dx%d' % (600, 400)) #MenuPage的窗口大小
        self.create_page()
        self.input_page = InputFrame(self.root) #增
        self.query_page = QuerryFrame(self.root)  #查
        self.delete_page = DeleteFrame(self.root) #删
        self.update_page = UpdateFrame(self.root) #改
        self.about_page = AboutFrame(self.root) #补充
        self.input_page.pack() #pack 可以理解为设置窗口首页
